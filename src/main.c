...
uint64_t __attribute__((aligned(4096))) s1_l1_tbl[512];
...
uint64_t __attribute__((aligned(4096))) s2_l1_tbl[512];
...
void handle_trap(vcpu_t *vcpu) {
    ...
    /* 0x18 hvc from aarch64 */
    if (ec == 0x18) {
        uart_puts("icevmm: guest called HVC! resuming...\n");
        vcpu->regs.elr_el2 += 4;    /* move past HVC instr */
    } else {
        uart_puts("icevmm: unhandled trap:\n");
        uart_puts("     ESR_EL2[");
        uart_put_hex(__esr);
        uart_puts("]\n");
        hang();
...
void main(void) {
    ...
    /* i mean this shouldn't ever be reached but here anyways :3 */
